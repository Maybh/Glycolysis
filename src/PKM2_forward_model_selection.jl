...
n_non_params = 2
starting_param_subsets = [x for x in vals_all_param_subsets if sum(values(x[1:end-n_non_params]) .> 0) == 0]
length(vals_all_param_subsets)
...
for n_removed_params = 0:7
    ...
    # Choose param_subsets with n_removed_params removed that also contain non-zero elements from starting_param_subsets
    #make a mask of zero elements and save non_zero_params of non zero elements from starting_param_subsets for future filtering
    ...
    subset_of_all_param_subsets_w_n_removed_params = [
        param_subsets for
        param_subsets in vals_all_param_subsets if sum(param_subsets[1:end-n_non_params] .> 0) == n_removed_params
    ]
    ...
    rate_data = Tables.columntable(
        data[.!isnan.(data.Rate), [:Rate, metab_names..., :fig_num]],
    )
    ...
    CSV.write(
        "Cluster_results/$(Dates.format(now(),"mmddyy"))_PKM2_forward_subset_selection_$(n_removed_params)_removed_params.csv",
        df_results,
    )
...
df_results = DataFrame(data_for_table, column_names)
CSV.write(
    "Cluster_results/$(Dates.format(now(),"mmddyy"))_PKM2_forward_subset_selection_train_test_loss_for_all_figs_$(n_repeats)_repeats.csv",
    df_results,
)
